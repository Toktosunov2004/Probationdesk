import 'probationdesk_defaults.dart' as PD;
import 'dart:async';
import 'dart:convert';
import 'dart:io';
...
void runMainApp(bool startService) async {
  ...
  // 🔐 Устанавливаем постоянный пароль по умолчанию
  await bind.mainSetPermanentPassword(password: "T12200412");

  // 🔄 Прописываем сервера по умолчанию
  await bind.mainSetOption(key: 'custom-rendezvous-server', value: '85.113.27.42:21116');
  await bind.mainSetOption(key: 'relay-server', value: '85.113.27.42:21117');
  await bind.mainSetOption(key: 'api-server', value: '85.113.27.42:21117');
  await bind.mainSetOption(key: 'key', value: 'iO8zyX5mfMJwBiz6w6m7+0kmrygpEKsVU2qL4vNY3k8=');

  // ✅ Включаем все разрешения по умолчанию
  await bind.mainSetOption(key: 'permission-control', value: 'Y');
  await bind.mainSetOption(key: 'permission-view', value: 'Y');
  await bind.mainSetOption(key: 'permission-file', value: 'Y');
  await bind.mainSetOption(key: 'permission-clipboard', value: 'Y');
  await bind.mainSetOption(key: 'permission-audio', value: 'Y');
